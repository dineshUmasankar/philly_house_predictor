...
In the course of this project, navigating the OPA's metadata presented challenges, particularly in discerning whether a given attribute should be treated as nominal or ordinal. The absence of clear descriptors necessitated interpretation based on feature language, making decision-making intricate. A notable lesson learned was the importance of actively calculating correlations early on, facilitating meaningful dimensionality reductions and informed column retention decisions, particularly with respect to the target variable, market_value.

One notable strength in the approach was the application of the naive model before hyperparameter tuning, saving considerable computational time during training. However, a reflection on the encoding strategy for categorical variables, such as the binary encoding for zip codes, revealed potential pitfalls in creating false relationships within the models. A future direction involves exploring alternative encoding methods to preserve information more effectively and reduce unnecessary features, addressing the specific issue encountered with the zip_code_0 encoding.

Cleaning outliers using winsorizing, while effective, raised concerns about losing valuable information and altering the dataset's distribution. Future investigations will focus on employing statistically robust techniques to handle outliers without sacrificing the overall dataset characteristics.
